...
export default function Filters({ open, onClose, onApply, onReset, initialState }) {
  ...
  useEffect(() => {
    if (!open) return;
    const { level: lv, time: tm, platforms: pf, tags: tg } = initialState || {};
  ...
  const handleApply = () => {
    onApply?.({ level, time, platforms: Array.from(platforms), tags: Array.from(tags) });
    onClose?.();
  };
        ...
        <header className="filters-header">
          ...
          <div className="filters-actions">
            <PixelButton className="btn-reset cursor-target" onClick={handleReset}>
              RESETEAR<Icon icon="pixelarticons:trash" width="16" height="16" />
            </PixelButton>
              ...
              {platformItems.map((p) => {
                ...
                return (
                  <button
                    ...
                    title={p.name}
                  >
                    <img src={platformImages[p.id]} alt={p.name} className="plat-icon" />
                  </button>
                );
              })}
            </div>
