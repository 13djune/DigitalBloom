...
export default function DataDotGrid({
  ...
  /* ---------- Normalización + filtrado (blindado) ---------- */
  ...
  /* ---------- Layout de nodos (centrado y compacto) ---------- */
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      dotsRef.current.forEach(dot => {
        ...
        ctx.save(); ctx.translate(ox, oy); ctx.scale(dotSize / 2, dotSize / 2); ctx.fillStyle = fill; ctx.fill(circlePath); ctx.restore();
      });
      nodesRef.current.forEach(n => {
        ...
        ctx.save(); ctx.translate(x, y); ctx.scale(R / 2, R / 2); ctx.fillStyle = n.color; ctx.fill(circlePath); ctx.restore();
        if (isHover || isActive) {
          ctx.beginPath(); ctx.arc(x, y, (R / 2) * 0.85, 0, Math.PI * 2); ctx.strokeStyle = isActive ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.85)'; ctx.lineWidth = 2; ctx.stroke();
        }
      });
  ...
  useEffect(() => {
    if (tooltip && tooltipRef.current && !tooltip.style) {
      ...
      setTooltip(prev => ({ ...prev, style: { left: `${finalX}px`, top: `${finalY}px` }, lineEndX, lineEndY: finalY + tipRect.height / 2 }));
    }
  }, [tooltip, tooltipOffset]);

  /* ---------- Layout cuando cambian datos/filtros ---------- */
  useEffect(() => {
          ...
          <>
            ...
            <div
              ...
              style={{
                ...
                gap: '16px',
                // ✅ AJUSTE ANTI-PARPADEO:
                // Si 'tooltip.style' no está calculado, lo renderiza invisible y fuera de pantalla.
                // Cuando se calcula, aparece instantáneamente en su sitio.
